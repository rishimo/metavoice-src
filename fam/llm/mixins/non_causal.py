...
class NonCausalInferenceMixin:
    ...
    def _non_causal_sample(
        ...
        *,
        ...
        ), f"input size {t} != config.block_size {self.config.block_size}"
        # forward the model to get the logits for the index in the sequence
        list_logits, _ = self(idx, speaker_embs=speaker_embs)  # c x (b, t, vocab_size)

        # scale by desired temperature
        list_logits = [
            logits / temperature for logits in list_logits
        ]  # c x (b, t, vocab_size)
        ...
        for b_prob in probs:  # c x (b, t, top_k) -> (b, t, top_k)
            out = [
                torch.multinomial(prob, num_samples=1).transpose(0, 1).unsqueeze(0)
                for prob in b_prob
